...
test('empty workbench navigates, validates hosts and preserves saved metadata', async () => {
  ...
  const launch = () => electron.launch({ args: ['.'], env: { ...process.env, SRE_DATA_DIR: data } });
  ...
  try {
    ...
    await page.getByLabel('登录密码', {exact:true}).fill('test-password-not-a-real-secret');
    await page.getByRole('button', { name: '保存', exact: true }).click();
    ...
    await expect(reopened.getByText('测试 Linux', { exact: true })).toBeVisible();
